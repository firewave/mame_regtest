...
static void print_driver_info(struct driver_entry* de, FILE* print_fd)
{
	...
	if( images ) {
		if( (images->device_briefname && xmlStrlen(images->device_briefname) > 0) &&
			...
			fprintf(print_fd, " (%s %s:%s)", images->device_briefname, images->device_file, images->device_part);
		}
	}
}
...
static char* create_commandline(struct driver_entry* de)
{
	...
	if(images) {
		if( (images->device_briefname && xmlStrlen(images->device_briefname) > 0) && 
			...
			if( images->device_part && xmlStrlen(images->device_part) > 0) {
				append_string(&sys, ":");
				append_string(&sys, (const char*)images->device_part);
			}
		}
...
static void execute_mame3(struct driver_entry* de, struct driver_info* actual_driv_inf)
{
	...
	if( config_use_softwarelist && config_hashpath_folder && actual_driv_inf->has_softlist ) {
		...
		xmlDocPtr soft_doc = xmlReadFile(driver_softlist_file, NULL, XML_PARSE_DTDATTR | XML_PARSE_DTDVALID);
		if( soft_doc ) {
...
{
	xmlDocPtr doc = xmlReadFile(filename, NULL, XML_PARSE_DTDATTR | XML_PARSE_DTDVALID);
	if( doc ) {
